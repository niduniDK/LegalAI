...
export function DocumentSummaryChatInterface({ documentContext }) {
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        'Triggering handleDocumentSummary for:',
        documentContext.filename
      );
  ...
  const handleDocumentSummary = async () => {
    ...
    console.log(
      'Starting document summary generation for:',
      documentContext.filename
    );
    ...
    try {
      ...
      const summaryResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/summary/generate`,
        {
          method: 'POST',
          headers: {
      ...
      if (!summaryResponse.ok) {
        ...
        throw new Error(
          `HTTP ${summaryResponse.status}: ${JSON.stringify(errorData)}`
        );
      }
      ...
      const errorMessage = {
        id: (Date.now() + 1).toString(),
        content:
          'Sorry, I encountered an error while generating the document summary. Please try again.',
        role: 'assistant',
        timestamp: new Date(),
      };
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/chat/get_ai_response`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      <div className='flex items-center justify-center h-screen'>
        <div className='text-center'>
          ...
          <p className='text-muted-foreground mb-4'>
            Please select a document from the discover page.
          </p>
              ...
              <div className='flex-1 min-w-0'>
                ...
                <div className='flex items-center gap-2'>
                  <Button
                    ...
                    onClick={() =>
                      window.open(documentContext.document?.url, '_blank')
                    }
                  >
            ...
            <div className='max-w-4xl mx-auto'>
              {messages.length === 0 && !isLoading && (
                <div className='text-center text-muted-foreground py-8'>
                  ...
                  <p className='text-sm'>
                    Please wait while I analyze the document and generate a
                            ...
                            <div className='grid gap-3 sm:grid-cols-1 md:grid-cols-2'>
                              {resources.map((url, idx) => {
                                ...
                                return (
                                  <Card
                                    key={idx}
                                    className='overflow-hidden hover:shadow-md transition-all duration-200 border-border/50 hover:border-border cursor-pointer'
                                  >
                                    <CardContent
                                      className='p-3'
      ...
      <div className='border-t border-border/40 p-4 flex-shrink-0'>
        <div className='max-w-4xl mx-auto'>
          ...
          <p className='text-xs text-muted-foreground mt-2 text-center'>
            You can ask follow-up questions about this document or request
            additional analysis.
          </p>
        </div>
      </div>
